...
const { host, fps, output } = yargs(hideBin(process.argv))
  ...
  .example(
    "iracing-driver-swaps --host 192.168.4.33:8182 --fps 1 --output output.txt",
...
const socketUpdateLogger = createLogger({
  level: "info",
  defaultMeta: { service: "iracing-driver-swap-cli", name: "socket-update" },
  transports: [
    consoleTransport,
...
const socketMetaLogger = createLogger({
  level: "info",
  defaultMeta: { service: "iracing-driver-swap-cli", name: "socket-meta" },
