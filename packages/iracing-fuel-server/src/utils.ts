...
export const normalizeFuelDataString = (
  dataString = "",
): [number, number[]] => {
...
export const createFuelParser = async (): Promise<ConfigIniParser> => {
  const fuelDataFileContent = await fs.promises.readFile(
    IRacingFuelDataFilePath,
    "utf-8",
  );
...
export const parseFuelDataString = (
  parser: ConfigIniParser,
  carName: string,
  track: string,
): string | undefined => {
  return parser.isHaveOption(carName, track)
    ? parser.get(carName, track)
    : undefined;
};
...
export const averageFuelData = (usage: number[]): number | undefined => {
  if (usage.length > 0) {
    const averageUsage =
      usage.reduce((averageUsage, usage) => averageUsage + usage, 0) /
      usage.length;
