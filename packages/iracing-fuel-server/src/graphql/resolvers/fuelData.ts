...
export const tracksForCarResolver = async (
  _,
  { carName },
): Promise<string[]> => {
  const parser = await createFuelParser();
...
export const isAverageUsageReliableResolver = async (
  _,
  { input: { track, carName } },
): Promise<boolean> => {
...
export const fuelUsageResolver = async (
  _,
  { input: { carName, track } },
): Promise<number[]> => {
...
export const averageFuelUsageResolver = async (
  _,
  { input: { track, carName } },
...
export const lastFuelUsageResolver = async (
  _,
  { input: { track, carName } },
): Promise<number | undefined> => {
  const [, usage] = parseFuelData(await createFuelParser(), carName, track);
